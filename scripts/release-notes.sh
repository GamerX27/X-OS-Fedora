...
FEDORA=44
EVENT="${EVENT:-push}"
REPO="${GITHUB_REPOSITORY:-GamerX27/X27-Linux}"
...
HIGHLIGHTS=(
  "base:fedora-release-common" "base:kernel-cachyos" "lts:kernel-cachyos-lts" "base:plasma-workspace"
  "base:mesa-dri-drivers" "base:systemd" "base:brave-origin" "base:rpm-ostree"
)
...
NOTES="${WORK}/notes.md"
{
  if [[ "$EVENT" == schedule ]]; then
    echo "Weekly rebuild: picks up the latest Kinoite ${FEDORA}, Brave and CachyOS kernel updates."
  else
  ...
  echo "| | Version |"
  echo "|---|---|"
  for entry in "${HIGHLIGHTS[@]}"; do
    name="${entry%%:*}"
    pkg="${entry#*:}"
    new="$(version_of "$pkg" "${WORK}/new/packages-${name}.txt")"
    [[ -n "$new" ]] || continue
    old=""
    [[ -n "$PREV_DIR" && -f "${PREV_DIR}/packages-${name}.txt" ]] && old="$(version_of "$pkg" "${PREV_DIR}/packages-${name}.txt")"
